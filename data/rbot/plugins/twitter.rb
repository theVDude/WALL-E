...
#  end
  ...
  def twitter(m, params)
    m.reply "http://twitter.com/thetalkingtwat - use @tweet to tweet shit"
  end
  ...
  def update_status(m, params)
    ...
    unless @registry.has_key?("Andrew_access_token")
       m.reply "You must first authorize your Twitter account before tweeting."
       return false;
    end
    @access_token = YAML::load(@registry["Andrew_access_token"])
...
plugin.map 'twitter', :action => "twitter"
plugin.map 'tweet *status', :action => "update_status", :threaded => true
